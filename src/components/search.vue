...
export default {
  ...
  methods: {
    ...
    goSearch(keywords) {
      let tempHistory = [{ first: keywords }, ...this.searchHistorys]
      this.searchHistorys = tempHistory.filter((item, index, self) => {
        return self.findIndex(i => i.first === item.first) === index
      })
